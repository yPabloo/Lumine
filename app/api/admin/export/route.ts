...
export async function GET() {
  ...
  try {
    ...
    const header = ["Código", "Criança", "Nascimento", "Responsável", "CPF", "E-mail", "Telefone", "Observação", "Status", "Pagamento", "Criada em"];
    const body = rows.map((row) => [row.code, row.childName, row.childBirthDate, row.guardianName, row.guardianCpf, row.guardianEmail, row.guardianPhone, row.notes, row.status, row.paymentStatus, row.createdAt].map(csvCell).join(";"));
